...
type Props = {
  ...
  period: string;
  items: ReportItem[];
  /** true (default) muestra el preview; false renderiza off-screen y deja
   *  solo el botón (vista de miembro). */
  preview?: boolean;
  buttonLabel?: string;
};

export function BudgetReportShare({
  localityName,
  period,
  items,
  preview = true,
  buttonLabel = "Compartir imagen",
}: Props) {
  ...
  if (budgeted.length === 0) {
    return (
      <p className="text-[12px] text-muted">
        No hay categorías con presupuesto definido para compartir.
      </p>
    );
  }
  ...
  const card = (
    ...
    >
      ...
      <div className="relative overflow-hidden bg-gold-grad px-6 pb-6 pt-7 text-white">
        ...
        <div className="relative mt-0.5 text-[12px] text-white/80">{period}</div>
      </div>
  ...
  return (
    <div>
      {preview ? (
        <div className="mx-auto w-full max-w-[400px]">{card}</div>
      ) : (
        <div
          aria-hidden
      ...
      <div className={preview ? "mt-4 flex justify-center" : "flex"}>
        <button
          type="button"
          onClick={handleShare}
          disabled={busy}
          className={`tap inline-flex items-center justify-center gap-2 rounded-xl bg-terra px-5 py-2.5 text-[13px] font-semibold text-white shadow-card-soft transition hover:bg-terra-light disabled:opacity-60 ${
            preview ? "" : "w-full"
          }`}
        >
