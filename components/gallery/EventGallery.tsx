...
import { getEventPhotoCount, getEventPhotos } from "@/lib/event-photos";
...
export async function EventGallery({ eventType, eventId }: Props) {
  const [previewPhotos, totalPhotos, session] = await Promise.all([
    getEventPhotos(eventType, eventId, { limit: PREVIEW_LIMIT }),
    getEventPhotoCount(eventType, eventId),
    getOptionalMember(),
  ]);
  ...
  const remaining = Math.max(0, totalPhotos - PREVIEW_LIMIT);
  const fullHref =
    eventType === "calendar"
      ...
      {totalPhotos === 0 ? (
        ...
        <div className="mb-4">
          ...
          {remaining > 0 && (
            ...
            >
              Ver galería completa ({totalPhotos} fotos) →
            </Link>
          )}
        </div>
      )}
