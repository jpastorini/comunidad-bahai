// home-refined.jsx — Hybrid: Luz golden header + Amanecer warmth, richer density

const hColors = {
  bg: '#F8F7F2',
  gold: '#C4A235', goldDark: '#96790E', goldLight: '#D4B85A',
  terra: '#2A3F8F', terraLight: '#3D56B0',
  amber: '#7E44B8',
  dark: '#2A2833',
  muted: '#7A7670',
  card: '#FFFFFF',
...
const HomeRefined = () => {
  const sections = [
    { icon: IconMensajes, title: 'Mensajes', sub: 'Casa Universal', c: hColors.terra, badge: 'Nuevo' },
    { icon: IconChat, title: 'Secretar\u00EDa', sub: 'Local', c: hColors.amber, badge: '2' },
    { icon: IconActividades, title: 'Actividades', sub: 'Locales', c: hColors.terra, badge: '3' },
    { icon: IconCalendario, title: 'Calendario', sub: 'Mayo 2026', c: hColors.amber },
    { icon: IconServicio, title: 'Servicio', sub: 'Necesidades', c: hColors.terra, badge: '5' },
    { icon: IconMateriales, title: 'Materiales', sub: 'de Estudio', c: hColors.amber },
    { icon: IconMetas, title: 'Metas', sub: 'Ense\u00F1anza', c: hColors.terra },
    { icon: IconTesoreria, title: 'Tesorer\u00EDa', sub: '65% meta', c: hColors.amber, progress: 0.65 },
  ];
  ...
  return (
    <div style={{
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '12px 13px 0' }}>
        ...
        {/* Featured Message Card (terracotta gradient) */}
        <div style={{
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 9, marginBottom: 12 }}>
          {sections.map((s, i) => {
            ...
            return (
              <div key={i} style={{
                background: hColors.card, borderRadius: 14,
                padding: '14px 10px 12px',
                display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 7,
                boxShadow: '0 2px 6px rgba(42,31,20,0.04)',
                position: 'relative',
              }}>
                {/* Badge */}
                {s.badge && (
        ...
        <div style={{ marginBottom: 14 }}>
          <div style={{
            fontSize: 14, fontWeight: 600, color: hColors.dark, marginBottom: 9,
          ...
          <div style={{ display: 'flex', gap: 9 }}>
            {activities.map((a, i) => (
              <div key={i} style={{
                ...
                <div style={{
                  fontSize: 12.5, fontWeight: 600, color: hColors.dark, lineHeight: 1.3, marginBottom: 3,
                }}>{a.title}</div>
                <div style={{
                  fontSize: 10.5, color: hColors.muted, fontFamily: "'Outfit', sans-serif",
                }}>{a.sub}</div>
              </div>
            ))}
          </div>
      ...
      {/* Tab Bar */}
      <TabBar activeIndex={0} accentColor={hColors.terra} bgColor={hColors.card} textColor={hColors.muted} />
    </div>
  );
};

Object.assign(window, { hColors, HomeRefined });
